window.translations_ja = {
    'page_title': 'Minh Triet - EdTech, Automation & AI for The new Generation',
    'nav_about': '概要', 'nav_experience': '経歴', 'nav_skills': 'スキル', 'nav_education': '学歴', 'nav_portfolio': 'プロジェクト', 'nav_collaboration': '協力', 'nav_contact': 'お問い合わせ',
    'nav_about_mobile': '概要', 'nav_experience_mobile': '経歴', 'nav_skills_mobile': 'スキル', 'nav_education_mobile': '学歴', 'nav_portfolio_mobile': 'プロジェクト', 'nav_collaboration_mobile': '協力', 'nav_contact_mobile': 'お問い合わせ',
    'hero_name': '教育、運営、テクノロジーの戦略的パートナー',
    "hero_subtitle_1": "| CEO, IVS JSC | AI & EdTechエキスパート | デジタルトランスフォーメーションリーダー |",
    'hero_title': '戦略的オペレーション & テクノロジーパートナー | デジタルトランスフォーメーション & パフォーマンス最適化',
    'hero_subtitle': 'テクノロジーアーキテクチャ（マイクロサービス、AIアダプティブラーニング）、EdTech製品開発（R&D）、および多業界の運用ガバナンス。',
    'hero_cta': 'Website IVS Academy',
    'hero_contact': '今すぐ連絡',
    'about_title': '私について',
    'about_p1': '私は戦略的経営と画期的技術を効果的に結びつける人物です。経営リーダーシップと多分野プロジェクト開発の経験を持ち、運用の卓越性と持続可能な成長の促進について深い理解を持っています。',
    'about_p2': '私はAIソリューションと自動化の導入・実装を専門とし、パフォーマンス向上、スケーラブルな製品開発、中核的競争優位性の構築を実現します。戦略的ビジョンを持つ企業の長期的発展に直接貢献する協業機会を常に探しています。',
    'portfolio_title': 'プロジェクト',
    'experience_title': '職歴',
    // EDUCATION SECTION KEYS
    'education_title': '学歴 & 資格',
    'edu1_degree': '経営管理学士',
    'edu1_institution': 'ベトナム航空アカデミー (ホーチミン市)',
    'edu1_duration': '2013年 - 2017年2月',
    'edu2_degree': '英語言語学士',
    'edu2_institution': 'ベトナム航空アカデミー (ホーチミン市)',
    'edu2_duration': '2012年 - 2016年9月',
    'edu3_degree': '一般教育管理資格証明書',
    'edu3_institution': 'ホーチミン市教育管理幹部学校 (ホーチミン市)',
    'edu3_duration': '2024年',
    'edu4_degree': '航空商業運営資格証明書',
    'edu4_institution': 'ベトナム航空アカデミー (ホーチミン市)',
    'edu4_duration': '2012年 - 2015年',

    /* New UI keys added for full localization */
    'filter_all': 'すべてのプロジェクト',
    'filter_business': 'ビジネス開発',
    'filter_education': '教育研究開発',
    'filter_utility': 'ユーティリティアプリケーション',
    'filter_website': 'ウェブサイト',
    'project1_title': 'IVS EdTech プラットフォーム', 'project1_desc': 'AIによる適応学習システムを備えた包括的な教育技術プラットフォーム。',
    'project2_title': 'IELTS Station センター - プロジェクト管理',
    'project2_desc': 'インフラ開発のための総合的なプロジェクト管理：市場調査、建設管理、施設計画、運用セットアップ。',
    'project3_title': 'AI アナリティクス ダッシュボード', 'project3_desc': 'ビジネスインテリジェンス向けの機械学習によるリアルタイム分析プラットフォーム。',
    'project4_title': 'IVS ENG', 'project4_desc': 'IVS Masteryカリキュラムに従ったK-12からの一般英語学習アプリケーション (ivseng.netlify.app)。',
    'project5_title': 'Mandarin by IVS', 'project5_desc': 'IVSが開発した中国語（普通話）学習サイト (mandarinbyivs.netlify.app)。',
    'project6_title': 'Strive Streak', 'project6_desc': '生産性／習慣トラッキングのウェブアプリ (strivestreak.netlify.app)。',
    'project7_title': 'QLGX', 'project7_desc': '教会、教区、信者などを管理するアプリケーション。',
    'project8_title': 'IVS StaySmart', 'project8_desc': '賃貸住宅管理アプリケーション、居住報告、部屋料金請求書などを統合。',
    'project9_title': '会議・結婚式センター', 'project9_desc': 'イベント＆結婚式会場のサイト (trungtamhoinghitieccuoi-banhanh.netlify.app)。',
    'project10_title': 'SuoiCamp', 'project10_desc': 'エコツーリズムウェブサイト、キャンプ、自然...',
    'project11_title': 'KhaDock', 'project11_desc': 'フロリダ州、米国での港湾建設請負業者のウェブサイト',
    'project12_title': 'KhaHome', 'project12_desc': 'フロリダ州、米国での内装建設、リノベーション、住宅アップグレード請負業者のウェブサイト',
    'project13_title': 'IVS Academy', 'project13_desc': 'IVS Academyの教育機関サイト (ivsacademy.edu.vn)。',
    'project14_title': 'Luv Yoga', 'project14_desc': 'ヨガスタジオおよびウェルネスのサイト (luv-yoga.site)。',
    // NEW PROJECTS ADDED HERE
    'project15_title': 'TheGenZiCon 2023 - プロジェクト管理',
    'project15_desc': '大規模な若者イベント（ドンナイ）を組織：300人以上の参加学生、決勝ナイトに1500人のゲストを集め、地区青年組合から表彰状を受領。',
    'project16_title': '観光パートナー管理（2022年）',
    'project16_desc': '観光、ホスピタリティ業界における戦略的パートナーシップの発展と維持に貢献し、運営の最適化とサービス収益の成長をサポート。',
    'stat_years_experience_label': '経験年数', 'stat_projects_completed_label': '完了したプロジェクト', 'stat_revenue_growth_label': '収益成長', 'stat_client_satisfaction_label': '顧客満足度',
    'stat_foreign_partners_label': '技術革新と導入',
    'scroll_indicator': 'スクロールして探索',
    'exp1_title': '最高経営責任者 (CEO)', 'exp1_company': 'IVS JSC (Integrate Vision Synergy 株式会社)', 'exp1_duration': '2024年 - 現在 | Đồng Nai', 'exp1_li1': '成果 & EdTech & AI製品開発:', 'exp1_li2': '  - AI & アダプティブラーニング統合: 人工知能 (AI) ソリューションの研究と実装により、パーソナライズされた学習体験 (アダプティブラーニング) を最適化し、生徒管理プロセスを自動化。', 'exp1_li3': '  - カリキュラム開発:', 'exp1_li4': '    • K-12カリキュラム (幼稚園から高校): すべてのK-12レベル向けの学術コンテンツの編集と開発に参加、デジタルプラットフォームの統合 (ivseng.netlify.app参照)。', 'exp1_li5': '    • 幼稚園カリキュラム: STEAM + Intelligenceを開発、子供の9種類の知能に適したSTEAM活動を組み合わせ。', 'exp1_li6': '    • ライフスキル & 英語カリキュラム: IVSLifeMindsを編集、小学生向けの専門英語と組み合わせたライフスキルカリキュラム。',
    'exp2_title': 'パートナーシップ＆プロジェクト開発責任者', 'exp2_company': 'AMG International Education JSC, Đồng Nai', 'exp2_duration': '2022年 - 2024年 | Đồng Nai', 'exp2_li1': '初年度で印象的な34%の収益成長を達成、継続的にビジネス目標を上回る。', 'exp2_li2': 'Long ThanhでのIELTS Station建設のプロジェクトマネージャー、市場調査、財務計画から語学センターの試験運用までの包括的な側面を担当。', 'exp2_li3': '教育管理: 旧Long Thanh地区の幼稚園から高校までのほぼ50の学校と連携、教育パートナーネットワークを拡大し、協力プログラムを運営。',
    'exp3_title': 'イベントプランニングマネージャー', 'exp3_company': 'TheGenZicon, Đồng Nai', 'exp3_duration': '2022年 - 2022年 | Đồng Nai', 'exp3_li1': '最終夜に300人以上の学生と1500人以上のゲストを引き付けるイベントを成功裏に組織 (DN RTVで報道)。', 'exp3_li2': 'Long Thanh地区青年連合から優れた組織成果で表彰状を受賞。',
    'exp4_title': 'ビジネス開発＆イベントマネージャー', 'exp4_company': 'Song Trang Restaurant', 'exp4_duration': '2021年 - 2022年 | Đồng Nai', 'exp4_li1': '観光パートナーシップ管理: 3ヶ月の観光で15,000人以上の顧客訪問とのパートナーシップを確立・維持、200社以上の旅行会社と接続、組織顧客ネットワークを拡大。', 'exp4_li2': 'レストラン＆イベント管理: グラフィックデザイナーの役割を引き受け、バンケットイベント、結婚式の装飾を管理、美的な品質と予算を確保。',
    'exp5_title': 'プロモーションマネージャー', 'exp5_company': 'The Grand Ho Tram Strip, HCMC (Ba Ria Vung Tau)', 'exp5_duration': '2018年 - 2018年 | Ba Ria Vung Tau', 'exp5_li1': 'プレミアム顧客ケア: メンバー顧客アカウントを管理し、貴重な顧客へのロイヤリティ報酬を計画、高品質な顧客サービスを確保 (Hospitality)。',
    'exp6_title': '客室乗務員', 'exp6_company': 'VietJet Air, Phu Quoc空港', 'exp6_duration': '2017年 - 2017年 | Phu Quoc', 'exp6_li1': '安全および緊急手順を厳格に実施、救急処置と高圧状況管理の集中トレーニングを受ける。', 'exp6_li2': '高品質な顧客サービス: プロフェッショナルな乗客サービス。',
    'skills_title': '専門分野',
    'skill_cat1_title': 'リーダーシップ & マネジメント', 'skill_cat1_li1': 'Cレベルのリーダーシップ (CEO/CTO/COO)', 'skill_cat1_li2': 'P&L管理', 'skill_cat1_li3': '複雑なプロジェクト管理',
    'skill_cat2_title': 'オペレーション & サービス', 'skill_cat2_li1': 'オペレーション管理', 'skill_cat2_li2': '語学センター運営', 'skill_cat2_li3': 'ハイエンド顧客サービス',
    'skill_cat3_title': '技術 & システム', 'skill_cat3_li1': 'マイクロサービスアーキテクチャ', 'skill_cat3_li2': 'Web/モバイル開発', 'skill_cat3_li3': 'SaaSプラットフォーム開発', 'skill_cat3_li4': 'DB設計',
    'skill_cat4_title': 'AI & 自動化', 'skill_cat4_li1': 'AI統合（アダプティブラーニング）', 'skill_cat4_li2': 'RPA', 'skill_cat4_li3': 'デジタルトランスフォーメーション',
    'skill_cat5_title': '教育 & 製品', 'skill_cat5_li1': 'カリキュラム開発（K-12、STEAM）', 'skill_cat5_li2': 'LMS/ERP/CRM実装', 'skill_cat5_li3': 'EdTech市場分析',
    'skill_cat6_title': 'パートナーシップ / カスタマーケア', 'skill_cat6_li1': '戦略的パートナーシップ開発', 'skill_cat6_li2': 'カスタマーケア管理', 'skill_cat6_li3': '顧客ニーズ分析',
    'collaboration_title': '協力可能',
    'collaboration_p1': '私は、自分の経験と専門知識をビジョナリーな企業に貢献する機会を探しています。以下の役割について話し合うことができます：',
    'collaboration_role1_title': '戦略的ポジション（フルタイム/パートタイム）', 'collaboration_role1_desc': '運用の運営と最適化、デジタルトランスフォーメーションと製品開発を主導するシニアリーダーシップの役割を引き受けます。',
    'collaboration_role2_title': '戦略的パートナー / アドバイザー', 'collaboration_role2_desc': '技術、ガバナンス、システムアーキテクチャ、およびEdTech製品戦略に関して企業が持続可能な成長を達成するのを支援します。',
    'collaboration_role3_title': 'ビジネスウェブサイトとアプリケーション', 'collaboration_role3_desc': 'ビジネスウェブサイトとアプリケーションを開発し、ユーザーエクスペリエンスを最適化し、特にビジネスの優先ニーズに合わせる。',
    'collaboration_role4_title': 'テクノロジー機器の供給', 'collaboration_role4_desc': 'ビジネスニーズに適したテクノロジー機器を相談、紹介、または提供する。',
    'collaboration_cta': '今すぐ接続',
    'contact_title': '連絡 & 議論',
    'contact_info_title': '連絡先情報',
    'contact_form_title': 'メッセージを送信',
    'form_name': 'フルネーム', 'form_email': 'メールアドレス', 'form_subject': '件名', 'form_message': 'メッセージ', 'form_submit': 'メッセージを送信',
    'form_sending': '送信中...', 'form_success': 'メッセージが送信されました！', 'form_error': 'エラーが発生しました。もう一度お試しください。',
    'form_subject_placeholder': '件名を選択...', 'form_subject_business': 'ビジネス協業', 'form_subject_project': 'プロジェクト提案', 'form_subject_speaking': '講演依頼', 'form_subject_other': 'その他',
    'contact_social_prompt': 'ソーシャルチャネルを通じて接続：',
    "footer_copyright": "© 2025 Nguyen Minh Triet. この資産はIVS JSCが所有しています。",
    'footer_credit': 'IVS Celestech と AI のコラボレーションによって設計され、ベトナムで ❤️ を込めて構築されました。',
    'footer_quick_links_title': 'クイックリンク',
    'footer_contact_title': '連絡先情報',
    'footer_contact_email': 'info@minhtriet.online',
    'footer_contact_phone': '+84 89692 0547',
    'footer_contact_address': 'ベトナム、ドンナイ省、ロンフオック',
    'footer_description': '戦略的オペレーション & テクノロジーパートナー。デジタルトランスフォーメーションとパフォーマンス最適化の専門家であり、画期的な技術ソリューションとスケーラブルな能力を創造することに情熱を注いでいます。',
    'ivs_toolbox_subtitle': '最高のAIプラットフォームのコレクション',
    'ivs_toolbox_description': 'AIは日々変化しています。私たちは最高のAIツールをまとめ、共有することで、あなたが自分のニーズに合ったソリューションを簡単に見つけられるように支援します。',
    'toggle_view_more': 'もっと見る (6+件)',
    'toggle_collapse': '少なく表示 (6+件を非表示)',
    'ivs_toolbox_cta': 'AIツールを探る'
};
